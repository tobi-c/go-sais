...
// sType
func (data byteData) makeSType() sType {
...
// bucket
func (data byteData) countToBucket(bkt bucket) {
	for i := range bkt {
		bkt[i] = 0
	}
...
func (data byteData) setBucketRight(bkt bucket) {
...
func (data byteData) setBucketLeft(bkt bucket) {
	data.countToBucket(bkt)
...
// induced sorting
func (data byteData) sortLType(sa suffixArray, sTy sType, bkt bucket) {
...
func (data byteData) sortSType(sa suffixArray, sTy sType, bkt bucket) {
...
func (data byteData) sortLMSTypeLMSPrefix(sa suffixArray, sTy sType, bkt bucket) {
	data.setBucketRight(bkt)
...
func (data byteData) sortLMSType(sa, lmsIndices suffixArray, bkt bucket) {
	data.setBucketRight(bkt)
...
func (data byteData) inducedSortLMSPrefix(sa suffixArray, sTy sType, bkt bucket) {
	data.sortLMSTypeLMSPrefix(sa, sTy, bkt)
	data.sortLType(sa, sTy, bkt)
	data.sortSType(sa, sTy, bkt)
}

func (data byteData) inducedSort(sa, lmsIndices suffixArray, sTy sType, bkt bucket) {
	data.sortLMSType(sa, lmsIndices, bkt)
	data.sortLType(sa, sTy, bkt)
	data.sortSType(sa, sTy, bkt)
}

func (data byteData) substringNames(sa, lmsSubstrs suffixArray, sTy sType) (suffixArray, int) {
	if len(lmsSubstrs) == 0 {
		return sa[:0], 0
	}
...
func (data byteData) nameToIndex(names, namesSA suffixArray, sTy sType) {
...
func (data byteData) sais(sa suffixArray, bktLen int) {
	if len(data) == 0 {
		return
	}
